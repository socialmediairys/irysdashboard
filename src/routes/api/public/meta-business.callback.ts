  ...
  server: {
    handlers: {
      GET: async ({ request }) => {
        ...
        if (pages.length === 0) {
          return html(
            "<h1>Nenhuma Página do Facebook encontrada</h1><p>Você precisa ser administrador de ao menos uma Página do Facebook com uma Conta Business do Instagram vinculada.</p><a href=\"/admin/configuracoes?tab=integracoes\">Voltar</a>",
            400,
          );
        }
        ...
        if (pages.length === 1) {
          ...
          return new Response(null, {
            status: 302,
            headers: { location: "/admin/configuracoes?tab=integracoes&meta=connected" },
          });
        }
        ...
        return new Response(null, {
          status: 302,
          headers: { location: "/admin/configuracoes?tab=integracoes&meta=choose-page" },
        });
      },
    },
  },
