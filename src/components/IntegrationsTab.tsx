...
export function IntegrationsTab() {
  ...
  const handleMConnect = async () => {
    setMBusy(true);
    try {
      const r = await startMetaAuth({ data: { origin: window.location.origin } });
      if (!r.ok) {
        toast.error(r.error);
        setMBusy(false);
        return;
  ...
  const handleMDisconnect = async () => {
    ...
    try {
      await disconnectM();
      toast.success("Meta Business desconectado");
      await refreshM();
    } catch (e) {
        ...
        <div className="flex items-start justify-between gap-3 flex-wrap">
          ...
          <div className="flex gap-2">
            {mLoading ? (
              ...
              <>
                <Button size="sm" variant="outline" onClick={refreshM} disabled={mBusy}>
                  <RefreshCw size={14} className="mr-1" /> Atualizar
                </Button>
                <Button size="sm" variant="outline" onClick={handleMDisconnect} disabled={mBusy}>
                  {mBusy ? <Loader2 className="h-4 w-4 animate-spin" /> : <LogOut size={14} className="mr-1" />}
                  Desconectar
                </Button>
              </>
            ) : (
              <Button size="sm" onClick={handleMConnect} disabled={mBusy}>
                {mBusy ? <Loader2 className="h-4 w-4 animate-spin" /> : <LinkIcon size={14} className="mr-1" />}
                Conectar
              </Button>
            )}
          </div>
