...
- Conteúdo (Fase 5): `conteudos` é a fonte única (global e tab do cliente); views são filtros; observações internas em `conteudos_internos`, versões imutáveis em `conteudo_versoes` criadas por trigger ao ir para `com_cliente` — why: aprovação da Fase 6 sem perder histórico e sem expor dados internos.
- Aprovações (Fase 6): cliente só lê `conteudos` enviados/`conteudo_versoes` via RLS e só avalia pela RPC `cliente_avaliar_conteudo` (valida dono, versão atual e comentário) — why: segurança não depende da interface. Tarefas ligam a conteúdo por `tarefas.conteudo_id` opcional.
